...
/// `TermStreamerBuilderWithState` is similar to `TermStreamerBuilder` but
/// also provides access to the automaton state when streaming.
/// This is useful for scoring based on automaton properties (e.g., Levenshtein distance).
pub struct TermStreamerBuilderWithState<'a, A>
where A: Automaton
{
    fst_map: &'a TermDictionary,
    stream_builder: StreamBuilder<'a, A>,
...
impl<'a, A> TermStreamerBuilderWithState<'a, A>
where
    A: Automaton,
    A::State: Clone,
{
    ...
    /// Creates the stream corresponding to the range
    /// of terms defined using the `TermStreamerBuilderWithState`.
    /// This converts to a state-aware stream at the final step.
    pub fn into_stream(self) -> io::Result<TermStreamerWithState<'a, A>> {
        Ok(TermStreamerWithState {
            fst_map: self.fst_map,
...
/// `TermStreamerWithState` is similar to `TermStreamer` but also provides
/// access to the automaton state for each matched term.
/// This is useful for computing scores based on automaton properties
/// (e.g., Levenshtein distance for fuzzy queries).
pub struct TermStreamerWithState<'a, A>
where
    A: Automaton,
{
    pub(crate) fst_map: &'a TermDictionary,
    pub(crate) stream: StreamWithState<'a, A>,
    term_ord: TermOrdinal,
    current_key: Vec<u8>,
    current_value: TermInfo,
...
impl<A> TermStreamerWithState<'_, A>
where
    A: Automaton,
    A::State: Clone,
{
    ...
    /// Returns the `TermOrdinal` of the given term.
    ///
    /// May panic if `.advance()` has never been called before.
    pub fn term_ord(&self) -> TermOrdinal {
        self.term_ord
    }
    ...
    /// by the `.next()` method.
    ///
    /// If the end of the stream has been reached, and `.next()`
    /// has been called and returned `None`, `.key()` remains
    /// the value of the last key encountered.
    ///
    /// Before any call to `.next()`, `.key()` returns an empty array.
    ...
    /// Accesses the current automaton state.
    ///
    /// Returns `None` if `.advance()` has never been called.
    /// This state can be used to compute scores based on automaton
    /// properties (e.g., Levenshtein distance).
    pub fn state(&self) -> Option<&A::State> {
        self.current_state.as_ref()
    }

    /// Return the next `(key, value, state)` tuple.
    #[expect(clippy::should_implement_trait)]
    pub fn next(&mut self) -> Option<(&[u8], &TermInfo, &A::State)> {
        if self.advance() {
            Some((self.key(), self.value(), self.current_state.as_ref().unwrap()))
        } else {
            None
        }
    }
